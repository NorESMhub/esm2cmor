...
c      WRITE(*,*) 'start read gridinfo done'
      CALL read_gridinfo_ifile
c      WRITE(*,*) 'read gridinfo done'
c
...
c
c --- --- Open output file
c          WRITE(*,*) 'BEFORE OPEN OFILE'
          IF (MOD(m-1,ramon).EQ.0) CALL open_ofile 
c          WRITE(*,*) 'AFTER OPEN OFILE'
c
...
c      ELSE
c        WRITE(*,*) 'WARNING: no lev coordinate found,read ilev'
...
c       CALL handle_ncerror(status)
      ENDIF
c     WRITE(*,*) 'start read P0'
c     status=nf90_inq_varid(ncid,'P0',rhid) 
c     CALL handle_ncerror(status)
c     status=nf90_get_var(ncid,rhid,p0)
c     CALL handle_ncerror(status)
c     WRITE(*,*) 'finish read P0'
      status=nf90_inq_varid(ncid,'ilev',rhid) 
      CALL handle_ncerror(status)
      status=nf90_get_var(ncid,rhid,ilev)
      CALL handle_ncerror(status)
...
c     CALL SYSTEM('rm '//TRIM(namelist_file_json))
c
...
c     table           = TRIM(tabledir)//"CMIP7_atmos2d.json"
c      table_id = cmor_load_table(table)
c      table_id = cmor_load_table(TRIM(tabledir)//"CMIP7_atmos2d.json")
c      table_id = cmor_load_table("CMIP7_atmos2d.json")
c      WRITE(*,*) 'loaded table:'
      iaxid=cmor_axis(
