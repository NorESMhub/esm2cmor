...
c        IF (.NOT.var_in_file(fnm,ivnm).AND..NOT.TRIM(ivnm).EQ.'transifs'
c     .    .OR.TRIM(ivnm).EQ.'transifs'.AND..NOT.lsecindex) CYCLE
        IF (.NOT.var_in_file(fnm,ivnm)) CYCLE
c
c --- - Prepare output file 
        CALL special_pre
...
c
     ...
     .  fac2)
c      IF (TRIM(ovnm).NE.'transifs') THEN
        IF(verbose) THEN
...
c      ENDIF
c
      IF (coord(1:1).EQ.'T'.AND.ovnm(1:6).NE.'sidmasstran') THEN
        grdid=cmor_grid(
     ...
     .    longitude_vertices=tlon_crnsp)
      ELSEIF (TRIM(ovnm).EQ.'sidmasstranx') THEN
        grdid=cmor_grid(
     .    axis_ids=(/iaxid,jaxid/),
     .    latitude=ulat,
     .    longitude=ulon,
     ...
     .    longitude_vertices=ulon_crnsp)
      ELSEIF (TRIM(ovnm).EQ.'sidmasstrany') THEN
        grdid=cmor_grid(
     .    axis_ids=(/iaxid,jaxid/),
     .    latitude=vlat,
     .    longitude=vlon, 
...
c      ELSEIF (TRIM(ovnm).EQ.'transifs') THEN
c        CONTINUE
      ELSE
        grdid=cmor_grid(
     .    axis_ids=(/iaxid,jaxid/),
