...
c
          ...
          IF (ind.GT.0) THEN
            ...
            IF (ind.GT.0) THEN
              ...
              IF (ind.GT.0) THEN
                ...
                vnm5=vnm5(1:ind-1)
                IF (INDEX(vnm6,'-').GT.0.AND.INDEX(vnm6,'+').GT.0) THEN
                  ind=MIN(INDEX(vnm6,'-'),INDEX(vnm6,'+'))
                ELSE
                  ind=MAX(INDEX(vnm6,'-'),INDEX(vnm6,'+'))
                ENDIF
                IF (PRESENT(fac7).AND.ind.GT.0) THEN
                  IF (vnm6(ind:ind).EQ.'-') fac7=-fac7
                  vnm7=vnm6(ind+1:smax)
                  vnm6=vnm6(1:ind-1)
                  IF (INDEX(vnm7,'-').GT.0.AND.INDEX(vnm7,'+').GT.0)
     .              THEN
                  ...
                  ENDIF
                  IF (ind.GT.0) THEN
                    IF (vnm7(ind:ind).EQ.'-') fac8=-fac8
                    vnm8=vnm7(ind+1:smax)
                    vnm7=vnm7(1:ind-1)
                    IF (INDEX(vnm8,'-').GT.0.AND.INDEX(vnm8,'+').GT.0)
     .                THEN
     ...
     .                THEN
                      ...
                      ENDIF
                      IF (ind.GT.0) THEN
                        IF (vnm9(ind:ind).EQ.'-') fac10=-fac10
                        vnm10=vnm9(ind+1:smax)
                        vnm9=vnm9(1:ind-1)
                        IF (INDEX(vnm9,'-').GT.0.AND.INDEX(vnm9,'+')
     .                     .GT.0) THEN
                        ...
                        ENDIF
                        IF (ind.GT.0) THEN
                          IF (vnm10(ind:ind).EQ.'-') fac11=-fac11
                          vnm11=vnm10(ind+1:smax)
                          vnm10=vnm10(1:ind-1)
                          IF (INDEX(vnm11,'-').GT.0.AND.
     .                        INDEX(vnm11,'+').GT.0) THEN
                            ind=MIN(INDEX(vnm11,'-'),INDEX(vnm11,'+'))
                          ELSE
                            ind=MAX(INDEX(vnm11,'-'),INDEX(vnm11,'+'))
                          ENDIF
                          IF (ind.GT.0) THEN
                            IF (vnm11(ind:ind).EQ.'-') fac12=-fac12
...
c
c      idx1 = index(vnm,'-')
c      idx2 = index(vnm(idx1+1:),'-') + idx1
c      cnm  = vnm(idx1+1:idx2-1)
c      WRITE(*,*) 'getvertcoord:tnm,vnm=',trim(tnm),'; ',trim(vnm)
...
c      c80=cval
      pdm=size(cval)
      DO k=1,pdm
        if (cval(k) .eq. 'alevel' .or. cval(k)(1:4) .eq. 'plev') then
          cnm = cval(k)
         EXIT
        ENDIF
      END DO
c      DEALLOCATE(cval)
c     stop "get_vertcoord"  ! debug removed or use single quotes if needed
c        c80=c80((INDEX(c80,' ')+1):)
c        ! in case zonal mean variables (second dimension plev)
c        IF (INDEX(C80,'plev').NE.1) THEN
c          c80=c80((INDEX(c80,' ')+1):)
c        ENDIF
c        IF (c80(1:4).NE.'time') THEN
c          cnm=c80(1:(INDEX(c80,' ')-1))
c        ELSE
c          cnm=c80((INDEX(c80,' ')+1):)
c        END IF
c
...
c      CHARACTER(len=:), ALLOCATABLE :: cval 
c
c      CALL json%initialize()
c      CALL json%load_file(filename=TRIM(tnm))
c
...
c      write(*,*) 'pdm:',pdm
      DO k=1,pdm
c        write(*,*) 'k:',k
c        write(*,*) 'cval(k):',trim(cval(k))
        if (cval(k)(1:4) .eq. 'time') then
          cnm = cval(k)
          EXIT
        ENDIF
      ENDDO
...
c     write(*,*) ibasedir,casename,itag
     ...
     .      mbnd,year,month)
        ...
        IF(LEN_TRIM(fnm).eq.0.and.INDEX(itag,'micom.').gt.0
     .    .and.isflag) THEN
          itag="blom."//itag(7:)
          isflag=.false.
        ELSE
...
c 
      INTEGER :: iind(*),jind(*),iflg(*),jflg(*),seclen
      REAL(KIND=8), DIMENSION(:,:) :: fldx,fldy
c
      INTEGER :: n
c
      transifs=0
...
c
      INTEGER :: idm,jdm
      REAL(KIND=8), DIMENSION(idm,jdm) :: angle,u,v
c
      INTEGER :: i,j
      REAL :: urot
c
...
c
      IMPLICIT NONE
c
      REAL(KIND=8) :: temp,saln,phiu,phil
c
      REAL(KIND=8) :: pu,pl,q,dphi,alpu,alpl
c
...
c
      REAL(KIND=8) th,s
      REAL(KIND=8) p1,p2,dphi,alp1,alp2
c
      REAL, PARAMETER ::
     . a11= 9.9985372432159340e-01,a12= 1.0380621928183473e-02
     .,a13= 1.7073577195684715e-03,a14=-3.6570490496333680e-05
