...
c --- Experiment namelist
     ...
     .  sub_experiment_id,parent_sub_experiment,parent_activity_id, 
     .  branch_method,parent_time_units,tracking_prefix,variant_label,
     .  source_type
      REAL(KIND=8), SAVE :: branch_time_in_child,branch_time_in_parent
      CHARACTER(LEN=slenmax), SAVE :: forcing_index,physics_index,
     .  realization_index,initialization_index 
      NAMELIST /experiment/ 
     .  activity_id, 
     .  variant_label,
...
c --- Tables 
     ...
     .  dEfx,dEyr,dEmon,dEmonZ,dEsubhr,dOclim,dOday,dOdaybgc,dOdec,
     .  dCOfx,dSIday
      CHARACTER(len=lenmax),DIMENSION(colmax,rowmax), SAVE :: 
     ...
     .  vEfx,vEyr,vEmon,vEmonZ,vEsubhr,vOclim,vOday,vOdaybgc,vOdec,
     .  vCOfx,vSIday
      CHARACTER(len=slenmax), SAVE ::
     ...
     .  pEfx,pEyr,pEmon,pEmonZ,pEsubhr,pOclim,pOday,pOdaybgc,pOdec,
     .  pCOfx,pSIday
      CHARACTER(len=slenmax), SAVE ::
     ...
     .  tEfx,tEyr,tEmon,tEmonZ,tEsubhr,tOclim,tOday,tOdaybgc,tOdec,
     .  tCOfx,tSIday
      INTEGER, SAVE ::
     ...
     .  nEfx,nEyr,nEmon,nEmonZ,nEsubhr,nOclim,nOday,nOdaybgc,nOdec,
     .  nCOfx,nSIday
      INTEGER, SAVE ::
     ...
     .  rEfx,rEyr,rEmon,rEmonZ,rEsubhr,rOclim,rOday,rOdaybgc,rOdec,
     .  rCOfx,rSIday
...
c
     ...
     .    TRIM(nmlfpexp)
      ...
      ENDIF  
c
c     CALL SYSTEM('cat '//TRIM(nmlfpvar))
...
c      CALL json%add(p,'forcing_index','f1')
c      CALL json%add(p,'forcing_index',1)
      CALL json%add(p,'parent_variant_label',TRIM(parent_variant_label))
      CALL json%add(p,'_controlled_vocabulary_file',
     .   'cmor-cvs.json')

c      CALL json%add(p,'_axis_entry_file',
c     .  TRIM(tabledir)//'CMIP7_coordinate.json')
      CALL json%add(p,'_AXIS_ENTRY_FILE','CMIP7_coordinate.json')

c      CALL json%add(p,'_formula_var_file',
c     .  TRIM(tabledir)//'CMIP7_formula_terms.json')
      CALL json%add(p,'_FORMULA_VAR_FILE','CMIP7_formula_terms.json')

c      CALL json%add(p,'_cmip6_option','CMIP6')
      CALL json%add(p,'_cmip7_option',1)
      CALL json%add(p,'activity_id',TRIM(activity_id))  
      CALL json%add(p,'source_type',TRIM(source_type))  
c      CALL json%add(p,'sub_experiment',TRIM(sub_experiment))
...
c      CALL json%add(p,'parent_activity_id',['CMIP'])
...
c      CALL json%add(p,'license',
c     . "CMIP6 model data produced by NCC is licensed under a Creative "
c     .//"Commons Attribution ShareAlike 4.0 International License "
c     .//"(https://creativecommons.org/licenses). Consult "
c     .//"https://pcmdi.llnl.gov/CMIP6/TermsOfUse for terms of use " 
c     .//"governing CMIP6 output, including citation requirements and " 
c     .//"proper acknowledgment. Further information about this " 
c     .//"data, including some limitations, can be found via the "
c     .//"further_info_url (recorded as a global attribute in this " 
c     .//"file) and at https:///pcmdi.llnl.gov/. "
c     .//"The data producers and data providers make no "
c     .//"warranty, either express or implied, including, but not " 
c     .//"limited to, warranties of merchantability and fitness for a "
c     .//"particular purpose. All liabilities arising from the " 
c     .//"supply of the information (including any liability arising " 
c     .//"in negligence) are excluded to the fullest extent permitted " 
c     .//"by law.") 
c 
...
c      CALL SYSTEM('cat '//TRIM(namelist_file_json))
c
